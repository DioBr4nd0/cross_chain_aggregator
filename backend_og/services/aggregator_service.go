...
func QueryDexRate(chainID, contractAddr, fromDenom, toDenom string) (math.LegacyDec, error) {
	chain, ok := config.GetChainByID(chainID)
	if !ok {
		return math.LegacyDec{}, fmt.Errorf("chain %s not found", chainID)
	}
	...
	if err != nil {
		return math.LegacyDec{}, fmt.Errorf("failed to connect to %s gRPC: %w", chainID, err)
	}
	...
	if err != nil {
		return math.LegacyDec{}, fmt.Errorf("failed to marshal query: %w", err)
	}
	...
	res, err := queryClient.SmartContractState(
		...
		&wasmtypes.QuerySmartContractStateRequest{
			...
			QueryData: queryMsgBz,
		},
	)
	if err != nil {
		return math.LegacyDec{}, fmt.Errorf("contract query failed: %w", err)
	}
	...
	if err := json.Unmarshal(res.Data, &rateResponse); err != nil {
		return math.LegacyDec{}, fmt.Errorf("failed to parse rate response: %w", err)
	}
	decRate, err := math.LegacyNewDecFromStr(rateResponse.Rate)
	if err != nil {
		return math.LegacyDec{}, fmt.Errorf("invalid rate format: %w", err)
	}

	return decRate, nil
}
func GetRatesForPairAcrossChains(fromToken, toToken string) ([]models.DexRateInfo, error) {
	var rates []models.DexRateInfo

	for _, chain := range config.GlobalChains {
		rate, err := QueryDexRate(chain.ID, chain.DexContract, fromToken, toToken)
		if err != nil {
			continue // Skip chains where pair isn't available
		}

		rates = append(rates, models.DexRateInfo{
			ChainID:    chain.ID,
	...
	if len(rates) == 0 {
		return nil, fmt.Errorf("no rates found for %s/%s", fromToken, toToken)
	}
